...
test "parseCommand defaults to help" {
    try std.testing.expectEqual(Command.help, try parseCommand(&.{"kuri-browser"}));
}

test "parseCommand handles standard flags" {
    try std.testing.expectEqual(Command.help, try parseCommand(&.{ "kuri-browser", "--help" }));
    try std.testing.expectEqual(Command.version, try parseCommand(&.{ "kuri-browser", "--version" }));
    try std.testing.expectEqual(Command.status, try parseCommand(&.{ "kuri-browser", "status" }));
    try std.testing.expectEqual(Command.roadmap, try parseCommand(&.{ "kuri-browser", "roadmap" }));
}

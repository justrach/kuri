...
fn spawnWorker(gw: *Gateway, sess: *Session, arena: std.mem.Allocator) bool {
    ...
    const bin_z = arena.dupeZ(u8, gw.cfg.worker_bin) catch return false;
    const port_z = arena.dupeZ(u8, port_str) catch return false;
    const token_z = arena.dupeZ(u8, sess.worker_token) catch return false;
    const home_z = arena.dupeZ(u8, home) catch return false;
    const state_z = arena.dupeZ(u8, state_str) catch return false;
